function init() {
	void 0 !== cRowId ? Public.ajaxPost("../basedata/contact.do?type=10&action=query", {
		id: cRowId
	}, function(a) {
...
function postData() {
	var a = "add" == oper ? "新增供应商" : "修改供应商",
		b = getData(),
		c = b.firstLink || {};
	delete b.firstLink, Public.ajaxPost("../basedata/contact.do?type=10&action=" + ("add" == oper ? "add" : "update"), b, function(d) {
		if (200 == d.status) {
	...
	Business.verifyRight("SUPPLYTYPE_ADD") && parent.$.dialog({
		...
		data: {
			...
			callback: function(a, b, c) {
				categoryCombo.loadData("../basedata/assist.do?action=list&typeNumber=supplytype", ["id", a.id]), c && c.api.close()
			}
		},
		width: 400,
		height: 100,

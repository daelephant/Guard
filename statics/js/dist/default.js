...
var dataReflush, list = {
	...
	assemble: {
		name: "组装单",
		href: "../scm/invOi?action=initOi&type=zz",
		dataRight: "ZZD_ADD",
		target: "storage",
		list: "../scm/invOi?action=initOiList&type=zz"
	},
//	disassemble: {
//		name: "拆卸单",
...
//	},
	...
	//accountTransfer: {
//		name: "资金转账单",
//		href: "../scm/fundTf?action=initFundTf",
//		dataRight: "ZJZZ_ADD",
//		target: "money",
//		list: "../scm/fundTf?action=initFundTfList"
//	},
	// puOrderTracking: {
	// 	name: "采购订单跟踪表",
...
$("#nav").on("click", "[rel=pageTab]", function(a) {
	...
	markupVension(), a(), $("#skin-" + SYSTEM.skin).addClass("select").append("<i></i>"), $("#sysSkin").powerFloat({
		...
		Public.ajaxPost("../basedata/systemProfile?action=changeSysSkin", {
			skin: a
		}, function(a) {
	...
	if ($("#scollUp").click(function() {
		...
		a.preventDefault(), $.dialog({
			...
			height: 180,
			title: "系统提示",
			content: '<div class="re-initialize"><h3>重新初始化系统将会清空你录入的所有数据，请慎重！</h3><ul><li>系统将删除您新增的所有商品、供应商、接收单位</li><li>系统将删除您录入的所有单据</li><li>系统将删除您录入的所有初始化数据</li></ul><p><input type="checkbox" id="understand" /><label for="understand">我已清楚了解将产生的后果</label></p><p class="check-confirm">（请先确认并勾选“我已清楚了解将产生的后果”）</p></div>',
			icon: "alert.gif",
			okVal: "重新初始化",
			ok: function() {

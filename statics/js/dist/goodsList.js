$(function() {
		...
		var a = Public.setGrid(f, g),
			...
			l = [{
				...
				formatter: function(a, b, c) {
					var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-pencil" title="修改"></span><span class="ui-icon ui-icon-copy" title="复制"></span><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pic" title="商品图片"></span></div>';
					return d
				},
				width: 120,
				title: !1
	...
	function c() {
		...
		}), $("#grid").on("click", ".operating .ui-icon-copy", function(a) {
			if (a.preventDefault(), Business.verifyRight("INVENTORY_ADD")) {
				var b = $(this).parent().data("id");
				h.operate("copy", b)
			}
		...
		h = {
			...
			del: function(a) {
				$.dialog.confirm("删除的商品将不能恢复，请确认是否删除？", function() {
					Public.ajaxPost("../basedata/inventory.do?action=delete", {
						id: a
					}, function(b) {
						if (b && 200 == b.status) {
							var c = b.data.id || [];
			...
			setStatus: function(a, b) {
				a && Public.ajaxPost("../basedata/inventory.do?action=disable", {
					invIds: a,
					disable: Number(b)
				}, function(c) {
			...
			setStatuses: function(a, b) {
				if (a && 0 != a.length) {
					var c = $("#grid").jqGrid("getGridParam", "selarrrow"),
						d = c.join();
					Public.ajaxPost("../basedata/inventory.do?action=disable", {
						invIds: d,
						disable: Number(b)
					}, function(c) {
						if (c && 200 == c.status) {
			...
			callback: function(a, b, c) {
				...
				d || (d = {}, $("#grid").data("gridData", d)), d[a.id] = a, "edit" == b ? ($("#grid").jqGrid("setRowData", a.id, a), c && c.api.close()) : "copy" == b ? ($("#grid").jqGrid("addRowData", a.id, a, "last"), c && c.api.close()) : ($("#grid").jqGrid("addRowData", a.id, a, "last"), c && c.resetForm(a))
			}
		},
		i = {
